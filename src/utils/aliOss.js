...
export default {
  ...
  createOssClient () {
    return new Promise((resolve, reject) => {
      let client = new OSS({
        ...
        accessKeyId: 'LTAI2IQtHd7JO6vv',
        accessKeySecret: 'jEycN8y2NirXXklkqtSvc7wirP7TBf',
        bucket: 'mallmytest'
      })
      resolve(client)
    })
  ...
  ossUploadFile (option) {
    ...
    const self = this
    return new Promise((resolve, reject) => {
      let date = DateUtils.dateFormat(new Date(), 'yyyyMMdd') // 当前时间
      let dateTime = DateUtils.dateFormat(new Date(), 'yyyyMMddhhmmss') // 当前时间
